...
class KabagADMController extends Controller
{
    public function tambahsurat()
    {
        $notadinas = Notadinas::all();
        return view('administrasi.tambahsurat', compact('notadinas'));
    }

    public function show()
    {
        $notadinas = Notadinas::whereIn('pengirim', ['Kepala Bagian ADM & Keu'])->get();
        return view('administrasi.infosuratnotadinas', compact('notadinas'));
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('administrasi.tambahsurat')->with('success', 'Nota Dinas berhasil dibuat.');
    }

    public function viewdispoadm($id)
    {
        ...
        public function downloadadm($id)
        {
            $direktur = Surat::find($id); // Menggunakan metode find() untuk mencari surat berdasarkan ID
            if (!$direktur) {
                return abort(404); // Mengembalikan 404 jika surat tidak ditemukan
